...
export const useAppStore = (): Omit<AppContextType, keyof ReturnType<typeof useDummyHandlers>> => {
  ...
  const [firewallHealth, setFirewallHealth] = useState<number>(100);

  // Cybersecurity Score State (Persisted locally)
  const [cyberScore, setCyberScoreState] = useState<number>(() => {
    try {
  ...
  const setCyberScore = useCallback((score: number) => {
    setCyberScoreState(score);
    try {
      localStorage.setItem('cyber_score', String(score));
    } catch (e) {}
  }, []);
  ...
  const applySessionState = useCallback((sessionState: SessionState) => {
    ...
    if (sessionState.cyberScore !== undefined) {
      setCyberScore(sessionState.cyberScore);
    } else {
      setCyberScore(0);
    }
  ...
  }, [currentSessionId, activeShaderCode, sliders, uniforms, cameraControlsEnabled, terraformConfig, controlConfig, soundConfig, sessionSource, shipConfig, canvasSize, viewMode, isHdEnabled, isFpsEnabled, isHudEnabled, collisionThresholdRed, collisionThresholdYellow, cyberScore]);
