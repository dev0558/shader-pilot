...
const CollisionPanel: React.FC = () => {
    ...
    return (
        <div className="space-y-4">
             <div className="flex items-start gap-3 p-3 bg-slate-900/80 rounded-lg border border-cyan-500/20 shadow-[0_0_12px_rgba(6,182,212,0.05)]">
                <div className="text-cyan-400 text-sm mt-0.5">
                ...
                <div className="space-y-1">
                    <h4 className="text-xs font-mono font-bold text-cyan-300 uppercase tracking-widest">Shield Barrier Sensors</h4>
                    <p className="text-[11px] text-gray-400 leading-relaxed font-sans">
                        Configure the proximity triggers for the ship's physical deflector fields. 'Critical Barrier Collision' halts translation immediately to protect hull indices, while the 'Yellow Warning Buffer' flashes warnings on standard telemetric feeds.
                    </p>
                </div>
            </div>
            <ControlSlider
                label="Critical Collision Threshold (Red Alert)"
                description="Trigger distance to high-density buildings that trips automatic emergency translation locks."
            ...
            />
            <ControlSlider
                label="Sensor Warning Range (Yellow Warning)"
                description="Outer distance limit that generates yellow HUD alert telemetry on approaches."
        ...
        <div className="flex-shrink-0 flex overflow-x-auto px-4 pt-2 gap-1 border-b border-gray-700 no-scrollbar">
            ...
            {EDITMODE && (
                <>
                    ...
                    {cameraControlsEnabled && <TabButton label="Integrity" isActive={activeTab === 'collision'} onClick={() => setActiveTab('collision')} />}
