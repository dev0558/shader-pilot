...
import type { Slider, SliderSuggestion, Modulation, CybersecurityQuestion } from '../types';
...
export const generateCyberQuestion = async (category?: string): Promise<CybersecurityQuestion> => {
    const topic = category || "general cybersecurity, systems hacking, defensive patches, web vulns, or network exploits";
    const prompt = `Generate a realistic, high-quality, professional multiple-choice cybersecurity scenario/question.
The topic should be relative to: ${topic}.
Provide a scenario describing a vulnerability discovery or an ongoing attack, optional codeSnippet of vulnerable code or logs, exactly 4 choice options, the index of the correct option (0-based), and a helpful explanation.
Be technical and human-readable. Do not use generic, overly simple descriptions. Make it sound like a real alert warning.`;

    const response = await ai.models.generateContent({
        model,
        contents: prompt,
        config: {
            systemInstruction: "You are an Elite DevSecOps and Cyber Incident Response Specialist. Give highly professional, deep, realistic scenarios.",
            responseMimeType: "application/json",
            responseSchema: {
                type: Type.OBJECT,
                properties: {
                    title: { type: Type.STRING, description: "Short high-tech name of the vulnerability or alert (e.g., 'SQLi Bypass', 'CSRF Remote Code Execution')" },
                    scenario: { type: Type.STRING, description: "Realistic scenario describing the incident, threat alert, security audit report, or server log snippet." },
                    codeSnippet: { type: Type.STRING, description: "Optional code snippet or raw log showing the vulnerability, command, or script. Leave blank if not applicable." },
                    options: {
                        type: Type.ARRAY,
                        items: { type: Type.STRING },
                        description: "Exactly 4 multiple choice options to patch, mitigate, or analyze this threat."
                    },
                    correctIdx: { type: Type.INTEGER, description: "Correct option index from 0 to 3." },
                    explanation: { type: Type.STRING, description: "Professional explanation on why this is the correct mitigation/remediation, and what the attack vector was." }
                },
                required: ['title', 'scenario', 'options', 'correctIdx', 'explanation']
            }
        }
    });

    return JSON.parse(response.text) as CybersecurityQuestion;
};

export const getCyberConsultantFeedback = async (
    threatTitle: string,
    scenario: string,
    codeSnippet?: string
): Promise<string> => {
    const prompt = `Perform a rapid Vulnerability and Exploit Threat analysis on:
Threat: ${threatTitle}
Scenario: ${scenario}
Code/Logs: ${codeSnippet || "None provided"}

Provide a concise, high-tech threat report containing:
1. RISK EVALUATION (LOW/MEDIUM/HIGH/CRITICAL)
2. IMMEDIATE MITIGATION STEPS (Actions to secure the server/network)
3. HINT/GUIDANCE for the operator on how to identify the correct remediation patch.

Keep it structured in raw markdown. Make it short, clean, and professional.`;

    const response = await ai.models.generateContent({
        model,
        contents: prompt,
        config: {
            systemInstruction: "You are the Shipboard Cyber Sentinel AI. You provide rapid security advisories and code analyses for pilots stranded in hostile cyber-zones.",
        }
    });

    return response.text || "Failed to analyze threat vector. Connection unstable.";
};
